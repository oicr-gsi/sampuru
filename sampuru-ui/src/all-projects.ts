...
export function activeProjects(projects: ActiveProject[]): HTMLElement {
  ...
  const welcomeText = elementFromTag("div",
   ...
   elementFromTag("p", null, "Case: The collection of: an individual sample received by OICR, and every QC-able which is extracted from that sample. A case is considered completed when all of the QC-ables associated with the case have their QC Status set to Passed QC."),
   elementFromTag("p", null, "QC-able: An individual unit of sample processing which can be assessed for quality control. A QC-able may be a tissue, an extracted stock, a library aliquot, or something else. The possible QC-able statuses are consistently colour-coded throughout Sampuru:"),
